...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Old Note Skin':
						ClientPrefs.oldnotes = !ClientPrefs.oldnotes;
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				switch(options[curSelected]) {
					...
					case 'Health Drain':
							var mult:Int = 1;
							if(holdTime > 1) { //Double speed after 1.5 seconds holding
								mult = 3;
							}
							ClientPrefs.healthdrain += add * mult;
							if(ClientPrefs.healthdrain < 47.5) ClientPrefs.healthdrain = 47.5;
							else if(ClientPrefs.healthdrain > 2000) ClientPrefs.healthdrain = 2000;
				}
				reloadValues();
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Dark UI':
				daText = "Add's a dark overlay, easier to read.";
			case 'FPS Counter':
			...
			case 'Old Note Skin':
				daText = "Bring back the vanilla ones";
			case 'Note Splashes':
		...
		for (i in 0...grpTexts.members.length) {
			...
			if(text != null) {
				...
				switch(options[textNumber[i]]) {
					...
					case 'Health Drain':
						if(ClientPrefs.healthdrain > 1999)
							daText = 'FC MODE';
						else
							daText = ClientPrefs.healthdrain + 'HP-';
				}
