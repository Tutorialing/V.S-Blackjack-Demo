...
class PlayState extends MusicBeatState
{
	...
	public var defaultCamZoom:Float = 1.05;
		...
		// blackjack shit here 
		// sprites
		var nozomybestboy:FlxSprite;
		var ruri:FlxSprite;
		var scareddemonman:FlxSprite;
		var boppers1dot0:FlxSprite;
		var boppers2dot0:FlxSprite;
		var busher:FlxSprite;
		var yoisthattabi:FlxSprite;
		var slotmachinebrokenaf:FlxSprite;
		var slotmachine:FlxSprite;
		var table:FlxSprite;
		var fronttable:FlxSprite;
	// mechanics
		public static var blackjackscream:Bool = false; 
		public static var blackjackinsanity:Bool = false;
		public static var shitshitshitshitshitshitshitshit:Bool = false;
		public static var subtocritva:Bool = false;
		public static var gamblemechanic = FlxG.random.int(1, 10);
		public static var bLackjackcallingyoualoser247 = FlxG.random.int(1, 8);
	// end
// end 
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'casino': 
			{ 
				defaultCamZoom = 0.65; // big zoom crit probably sleeping i pinged him like several times :sob:
				curStage = 'casino'; // it isn't hard

				var slychad = Paths.getSparrowAtlas('Bop2', 'shared');

				var infiniteandtheboys = Paths.getSparrowAtlas('Bop1', 'shared'); // 							var bestboy = Paths.getSparrowAtlas('BlackJack/animated/DEMON');

				var tabilmao = Paths.getSparrowAtlas('TabiLol', 'shared');

				var ruritex = Paths.getSparrowAtlas('FoxLady', 'shared');

				var bestboylesgoooo = Paths.getSparrowAtlas('Nozomy', 'shared');
				
				// wtf is scorll factor lmfaoofaoa
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('BG'));
				bg.scrollFactor.set(1, 1);
				bg.setGraphicSize(Std.int(bg.width * 1.1)); // for no funni haha black lines
				add(bg);

				slotmachinebrokenaf = new FlxSprite(-600, -200).loadGraphic(Paths.image('Broken'));
				slotmachinebrokenaf.scrollFactor.set(1, 1);
				slotmachinebrokenaf.alpha = 0;
				slotmachinebrokenaf.setGraphicSize(Std.int(slotmachinebrokenaf.width * 1.1));	
				slotmachinebrokenaf.antialiasing = true;
				add(slotmachinebrokenaf);

				// slotmachine = new FlxSprite(-600, -200).loadGraphic(Paths.image('Slot'));
				// slotmachine.scrollFactor.set(1, 1);
				// slotmachine.setGraphicSize(Std.int(slotmachine.width * 1.1));	
				// slotmachine.antialiasing = true;
				// add(slotmachine);

				boppers1dot0 = new FlxSprite(-400, 250);
				boppers1dot0.frames = infiniteandtheboys;
				boppers1dot0.animation.addByPrefix('PeepBop1', "PeepBop1", 24);
				boppers1dot0.animation.play('PeepBop1');
				boppers1dot0.antialiasing = true;
				add(boppers1dot0);

				boppers2dot0 = new FlxSprite(1750, 300);
				boppers2dot0.frames = slychad;
				boppers2dot0.animation.addByPrefix('PeepBop2', "PeepBop2", 24);
				boppers2dot0.animation.play('PeepBop2');
				boppers2dot0.antialiasing = true;
				add(boppers2dot0);

				yoisthattabi = new FlxSprite(-600, 300);
				yoisthattabi.frames = tabilmao;
				yoisthattabi.animation.addByPrefix('TabiLol', "TabiLol", 24);
				yoisthattabi.animation.play('TabiLol');
				yoisthattabi.antialiasing = true;
				add(yoisthattabi);

				table = new FlxSprite(-600, -200).loadGraphic(Paths.image('Table'));
				table.scrollFactor.set(1, 1);
				table.antialiasing = true;
				add(table);

				var liteshit = new FlxSprite(-600, -500).loadGraphic(Paths.image('lite'));
				liteshit.scrollFactor.set(1, 1);
				liteshit.antialiasing = true;
				add(liteshit);

				fronttable = new FlxSprite(-600, -200).loadGraphic(Paths.image('tablefront'));
				fronttable.scrollFactor.set(1, 1); 
				fronttable.antialiasing = true;

				nozomybestboy = new FlxSprite(1500, 700); //y was -400
				nozomybestboy.frames = bestboylesgoooo;
				nozomybestboy.animation.addByPrefix('DemonBop', "DemonBop", 24);
				nozomybestboy.animation.play('DemonBop');
				add(nozomybestboy);
				nozomybestboy.antialiasing = true;	
				
				ruri = new FlxSprite(-400, 600); //y was -300. x was -170
				ruri.frames = ruritex;
				ruri.animation.addByPrefix('RuriBop', "RuriBop", 24);
				ruri.antialiasing = true;
				add(ruri);
				ruri.animation.play('RuriBop');

				busher = new FlxSprite(-550, -300).loadGraphic(Paths.image('Bush')); //y was -250
				busher.scrollFactor.set(1, 1);
				busher.setGraphicSize(Std.int(busher.width * 1.1));	
				busher.antialiasing = true;			
				add(busher);	
			}
			case 'stage': //Week 1
	...
	function images(arg0:String):String {
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (ClientPrefs.oldnotes) 
				skin = 'NOTE_assets-OLD';
			else {
				skin = 'NOTE_assets';
			}
